function [returnEMG, returnOptions] = emgEventsDetectTool(EMG, options)
...
%       2. returnOptions: A matrix of detection options with columns
...
if(isvector(vars.options.subParams))
    ...
    if(size(vars.options.subParams, 2) == vars.numChannels)
        ...
        error('Invalid detection parameters provided. Number of columns indetection parameter matrix should be equal to number of EMG channels.');
    end
end
...
% Callback functions
    ...
    function runDetector(runForAll)
        ...
        if runForAll == 1
            for i = 1:vars.numChannels
                vars.detectionCellarray{i} = vars.options.detectionAlgoSteps(vars.channelStream(:, vars.channelNum),...
    ...
    function closeFigure(~,~)
        returnEMG = vars.EMG;
        for i=1:vars.numChannels
            returnEMG.events(i).onSets  = vars.detectionCellarray{vars.channelNum}{end, vars.ONSETS_COLUMN_NUM};
            returnEMG.events(i).offSets = vars.detectionCellarray{vars.channelNum}{end, vars.OFFSETS_COLUMN_NUM};
        end
        returnOptions = vars.detectionParams;
        delete(gcf);
    end
end